...
\- Missing values found in `ship\_mode`.
...
\- Filled missing `ship\_mode` values with "Unknown".
...
\- Missing `ship\_mode` values were filled with "Unknown" and flagged in the quality report.
